...
//            查询数据库中和输入的用户名一致的返回其对象ApUser
            ...
            if (!pwd.equals(dbUser.getPassword())) {//密码不正确
            ...
            Map<String, Object> map = new HashMap<>(); //定义map集合，同接口文档
            map.put("token", token); //接口文档中的data下属性值是"token"
            dbUser.setSalt("");   //不需要返回给前端的值
            dbUser.setPassword("");
            map.put("user", dbUser); //接口文档data下JSON格式的user
